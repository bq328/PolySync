...
定义 PolySync **网站化（Web）+ 本机 Agent** 的产品与技术架构，在 **平台不托管用户私钥** 的前提下，实现全自动 Live 跟单。
...
| 模型 | 平台能否碰私钥 | 全自动 Live | 用户门槛 | PolySync 主线 |
|------|----------------|-------------|----------|-----------------|
...
┌─────────────────────────────────────────────────────────────────┐
│ L4  用户界面层                                                    │
│     Web App (app.PolySync.com) · 可选移动端 · Telegram 入口     │
└───────────────────────────────┬─────────────────────────────────┘
                                │ HTTPS / WSS（无私钥）
┌───────────────────────────────▼─────────────────────────────────┐
...
| **B — PolySync Cloud** | 账号、配置 JSON、观察池、活动流副本 | ❌ |
| **C — 浏览器** | JWT、UI | ❌ |
...
| 延迟 | 最低 |
| 与现 PolySync | ≈ 现有 daemon + `uplink` + `pairing` |
...
Base URL: `https://api.PolySync.example`（示例）
...
→ 200
{
  "deviceToken": "…",
  "deviceId": "…",
  "wsUrl": "wss://ws.PolySync.example/v1/agent",
  "userId": "…"
}
```
...
| UI | `127.0.0.1` Dashboard | `app.PolySync.com` |
| 私钥 | `.env` | Keychain / `.env` |
| 多设备看数据 | 难 | 易（云端同步活动流） |

**C 不是替代 B，是 B 的产品化上层：** Agent 内核可以是 PolySync daemon 的演进包。
...
### 13.2 平台侧 Cloud（由 PolySync 运营）

| 服务 | 示例主机名 |
|------|------------|
| Web SPA | `app.PolySync.example` |
| Control Plane API | `api.PolySync.example` |
| Agent WebSocket | `ws.PolySync.example` |
...
PolySync 现 Live 路径（`src/executor/secure-client.ts`）使用 `Wallet(privateKey)`，与官方模型一致。
...
- [ ] `PolySync-agent` CLI 包（= 现 daemon + 结构化 Agent 入口）
...
与 [ECOSYSTEM_WORKFLOW.md](ECOSYSTEM_WORKFLOW.md) 中 Predicts.guru → PolyWallet → PolySync 流程一致；Web 负责配置与可见性，Agent 负责执行。
...
**免责声明：** PolySync 为跟单执行与风控工具，不构成投资建议。Web + Agent 架构旨在减少平台托管私钥的风险，**不能**消除预测市场亏损、Agent 设备被入侵或用户操作失误等风险。请使用专用小额钱包并先在 Preview 模式下充分验证。
