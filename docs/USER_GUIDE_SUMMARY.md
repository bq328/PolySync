...
PolySync 在 Polymarket 上跟踪多个 Leader 的成交，按你的策略缩放后自动跟单。默认 **Preview 模拟**，不真实下单。
...
**正常：** 日志 `PolySync starting`，Leader 成交时出现 `PREVIEW would copy`。
