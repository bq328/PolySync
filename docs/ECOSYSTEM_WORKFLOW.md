...
PolySync **不应**做成第二个 Predicts.guru 或 PolyWallet，而应占住：

> **研究之后 → Preview 验证 → Live 执行**

| 工具 | 定位 | 核心场景 | PolySync 是否重做 |
|------|------|----------|---------------------|
...
| **PolySync** | 自托管跟单引擎 | 观察池 → Preview → Live 执行 + 风控 | ✅ 核心 |
...
PolyWallet        观察 7～14 天（PnL 日历、Live 持仓）
       ↓
PolySync        观察池 → Preview 7 天 → Live 小额
       ↓
预测市场实验室     内容教这条流程，PolySync 作 Demo 工具
```
...
| ④ 模拟 | PolySync Preview | 模拟跟单，看 COPY / SKIP | Preview 报告 |
| ⑤ 执行 | PolySync Live | 小额 + Kill Switch | 真实跟单 + 活动流审计 |
...
┌────────────────────────────▼────────────────────────────────┐
│              预测市场实验室（B 站 / YouTube）                  │
│  教流程 │ 录 Preview 报告 │ PolySync 作案例工具               │
└─────────────────────────────────────────────────────────────┘
```
...
把 Predicts.guru 筛人、PolyWallet 观察、PolySync 跟单 **串成一条线**。
...
> PolyWallet 无法提供「若跟单会怎样」的 counterfactual；这是 PolySync 独有能力。
...
| CLI：`PolySync inspect <wallet>` | 终端版 Leader 快查 |
...
| 视频选题 | 主要工具 | PolySync 露出 |
|----------|----------|-----------------|
| Predicts.guru 筛 Leader 全流程 | Predicts.guru | 弱 |
| PolyWallet 观察 14 天 | PolyWallet | 弱 |
| Preview 7 天实录 | PolySync | **强**（Preview 报告） |
| 多 Leader 风控配置 | PolySync | **强** |
...
- [ ] Onboard 向导 + 外链工具链  
- [ ] 可拍摄：Predicts → PolyWallet → PolySync 三期系列  
...
> **PolySync = 自托管 Polymarket 跟单引擎，带「观察 → Preview 验证 → Live 执行」完整工作流；研究用 Predicts.guru，盯 PnL 用 PolyWallet，跟单决策与执行用 PolySync。**
...
**免责声明：** PolySync 为跟单执行与风控工具，不构成投资建议。预测市场存在亏损风险。Predicts.guru、PolyWallet 等为第三方服务，使用前请自行评估其数据准确性与服务条款。
