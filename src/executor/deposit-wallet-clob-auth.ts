...
const CLOBAUTH_TYPE_HASH = keccakUtf8(CLOBAUTH_TYPE_STRING);
const DOMAIN_TYPE_HASH = keccakUtf8("EIP712Domain(string name,string version,uint256 chainId)");
const CLOBAUTH_DOMAIN_NAME_HASH = keccakUtf8("ClobAuthDomain");
const CLOBAUTH_DOMAIN_VERSION_HASH = keccakUtf8("1");
...
function wrapPoly1271Signature(
  ...
  const contentsTypeHex = hexlifyUtf8(CLOBAUTH_TYPE_STRING);
...
export async function deriveDepositWalletClobCredentials(
  ...
  const eoaSig = await signTypedData(
    privateKey,
    typedData.domain,
    typedData.types,
    typedData.message
